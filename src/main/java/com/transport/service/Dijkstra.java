...
public class Dijkstra {
    // It determines the fastest path between two cities
    public static void findFastestPath(Graph graph, String startCity, String endCity) {
        // We verify if the cities are in the our map
        Map<String, Node> nodes = graph.getNodes();
        if(!nodes.containsKey(startCity) || !nodes.containsKey(endCity)){
            System.out.println("Error: One or two cities weren't found");
            return;
        }
        ...
        while (!pq.isEmpty()) {
            ...
            // If we arrive to the destiantion, we can stop
        ...
        printResults(startCity, endCity, minTimes.get(endNode), parents, endNode);
    }

    private static void printResults(String start, String end, double totalTime, Map<Node, Node> parents, Node endNode) {
        if (totalTime == Double.MAX_VALUE) {
            System.out.println("There isn't road between " + start + " and " + end);
            return;
        }
        ...
        Collections.reverse(path); // Reverse the list to have Start -> Destinațion
        ...
        System.out.println("\n========= NAVIGATION RESULTS =========");
        System.out.println("Optimal route: " + String.join(" -> ", path));
        System.out.println("Estimated total time: " + hours + "h " + minutes + "min");
        System.out.println("=====================================\n");
    }
}
